...
## DESCRIPTION
##
##   WHAT can be `via` or `demo`.  TARGET is one of the pages in
##   `src/html` such as `audio_annotator` or `image_annotator`.
    ...
    with open(out_html, 'w') as outf, open(target_html, 'r') as inf:
        for line in inf:
            if '<script src="' in line:
                tok = line.split('"')
                filename = tok[1][3:]
                write_file_contents(outf, 'script', os.path.join(SRC_DIR, filename))
            elif '<link rel="stylesheet" type="text/css"' in line:
                tok = line.split('"')
                filename = tok[5][3:]
                write_file_contents(outf, 'style', os.path.join(SRC_DIR, filename))
